...
export const Markdown = (props: MarkdownProps): JSX.Element => {
  return (
    <ReactMarkdown className="break-words" remarkPlugins={[remarkGfm, remarkSlug]} children={props.markdownSource} components={{
      p: ({ node, ...props }) => {
        return (
          <p className="inline-block" {...props}></p>
        )
      },
      h1: ({ node, children, ...props }) => {
        return (
          <h1 className="font-semibold text-5xl my-4" {...props}>{children}</h1>
        )
      },
      h2: ({ node, children, ...props }) => {
        return (
          <h2 className="font-semibold text-4xl my-4" {...props}>{children}</h2>
        )
      },
      h3: ({ node, children, ...props }) => {
        return (
          <h3 className="font-semibold text-3xl my-4" {...props}>{children}</h3>
        )
      },
      h4: ({ node, children, ...props }) => {
        return (
          <h4 className="font-semibold text-2xl my-4" {...props}>{children}</h4>
        )
      },
      h5: ({ node, children, ...props }) => {
        return (
          <h5 className="font-semibold text-xl my-4" {...props}>{children}</h5>
        )
      },
      h6: ({ node, children, ...props }) => {
        return (
          <h6 className="font-semibold my-4" {...props}>{children}</h6>
        )
      },
      strong: ({ node, ...props }) => {
        return (
      ...
      blockquote: ({ node, ...props }) => {
        return (
          <blockquote className="bg-white border-l-8 border-yellow-300 px-4 py-3 my-4 mx-1" {...props}></blockquote>
        )
      },
      ...
      a: ({ node, ...props }) => {
        return (
          <a className="text-blue-600 hover:underline" {...props}>{props.children}</a>
        )
      },
      img: ({ node, ...props }) => {
        return (
          <span className="w-full flex justify-center"><img className="object-fill" alt={props.key} {...props} /></span>
        )
      },
      code: ({ node, inline, className, children, ...props }) => {
        const match = /language-(\w+)/.exec(className || '')
        if (!inline && match) {
          return <SyntaxHighlighter className="rounded-lg" style={vs2015} language={match[1]} children={String(children).replace(/\n$/, '')} />
        }

        return <span className="bg-yellow-200 text-yellow-800 px-1 rounded-md" {...props}>{children}</span>
      },
      table: ({ node, ...props }) => {
        return (
      ...
      del: ({ node, ...props }) => {
        return (
          <span className="line-through" {...props}></span>
        )
      },
    }} />
  );
